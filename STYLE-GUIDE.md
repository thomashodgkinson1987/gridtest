# GridTest Style Guide

This document is the single source of truth for all coding, documentation, and workflow conventions used in the GridTest project. Adhering to these standards ensures our codebase remains clean, consistent, and easy to navigate.
...
-   **Brace Style:** We use the **Allman style**, where the brace for a control statement (`if`, `else`, `for`, `while`) or function definition is always placed on a new line.
...
-   **Include Guards:** Use `#ifndef PROJECT_MODULE_H` format (e.g., `GRIDTEST_COMMAND_SYSTEM_H`).
...
-   **Capitalization:** The subject line begins with a capital letter.
...
### 4.3. Command Preferences

-   For changing branches, prefer `git switch` over `git checkout`.
